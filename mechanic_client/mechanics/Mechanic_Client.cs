...
namespace mechanic_client
{
    public class Mechanic_Client : Script
    {
        ...
        static string[] windowParts = new string[4]
       {
        ...
        public static void FixBodyShell(int veh)
        {
            int vehicle = veh;
            List<Vehicle> vehicles = Entities.Vehicles.All;
            Vehicle veh_obj = vehicles.Find(pl => pl.Handle == vehicle);
            RAGE.Game.Vehicle.SetVehicleFixed(vehicle);
        ...
        public static bool[] GetStatusWheel(int targetVeh)
        {
            int veh = targetVeh;
            for (int i = 0; i < 8; i++)
            {
            ...
            if (Wheel[0])
            {
                VehicleBone("wheel_lf", "~y~Колесо. ~r~Не исправно. ~r~[E] ~s~~y~Починить~y~");
            }
            else
            {
            ...
            if (Wheel[1])
            {
                VehicleBone("wheel_rf", "~y~Колесо. ~r~Не исправно. ~r~[E] ~s~~y~Починить~y~");
            }
            else
            {
            ...
            if (Wheel[4])
            {
                VehicleBone("wheel_lr", "~y~Колесо. ~r~Не исправно. ~r~[E] ~s~~y~Починить~y~");
            }
            else
            {
            ...
            if (Wheel[5])
            {
                VehicleBone("wheel_rr", "~y~Колесо. ~r~Не исправно. ~r~[E] ~s~~y~Починить~y~");
            }
            else
            {
            ...
            if (Wheel[8])
            {
                VehicleBone("wheel_lm", "~y~Колесо. ~r~Не исправно. ~r~[E] ~s~~y~Починить~y~");
            }
            else
            {
            ...
            if (Wheel[9])
            {
                VehicleBone("wheel_rm", "~y~Колесо. ~r~Не исправно. ~r~[E] ~s~~y~Починить~y~");
            }
            else
            {
        ...
        public static float GetEngineHealth(int targetVeh)
        {
            int veh = targetVeh;
            ...
            {
                VehicleBone("engine", "~y~Двигатель~y~. ~y~Повереждён ~r~[Z] ~s~ ~y~Починить~y~" + eHeatlh.ToString());
            }
            else if (eHeatlh <= 400)
            {
                VehicleBone("engine", "~y~Двигатель~y~. ~r~Неисправен ~r~[Z] ~s~ ~y~Починить~y~" + eHeatlh.ToString());
            }
            return eHeatlh;
        }


        public static void FixWheel(int veh)
        {
            int vehicle = veh;
            List<Vehicle> vehicles = Entities.Vehicles.All;
            Vehicle veh_obj = vehicles.Find(pl => pl.Handle == vehicle);
            ...
            for (int i = 0; i < wheelParts.Length; i++)
            {
                ...
                Vector3 coords = RAGE.Game.Entity.GetWorldPositionOfEntityBone(vehicle, index);
                ...
                if (RAGE.Game.Utils.Vdist(pos.X, pos.Y, pos.Z, coords.X, coords.Y, coords.Z) <= 1.5f)
                {
                    ...
                    {
                        RAGE.Game.Vehicle.SetVehicleTyreFixed(vehicle, 0);
                    ...
                    {
                        RAGE.Game.Vehicle.SetVehicleTyreFixed(vehicle, 1);
                    ...
                    {
                        RAGE.Game.Vehicle.SetVehicleTyreFixed(vehicle, 4);
                    ...
                    {
                        RAGE.Game.Vehicle.SetVehicleTyreFixed(vehicle, 5);
        ...
        public static bool[] GetWindow(int targetVeh)
        {
            int veh = targetVeh;
            if (RAGE.Game.Vehicle.IsVehicleWindowIntact(veh, 0))
            {
            ...
            {
                VehicleBone("window_lf", "~y~Окно. ~r~Не исправно. ~r~[J] ~s~~y~Починить~y~");
                Window[0] = true;
            }
            ...
            {
                VehicleBone("window_rf", "~y~Окно. ~r~Не исправно. ~r~[J]~s~~y~Починить~y~");
                Window[1] = true;
            }
            if (RAGE.Game.Vehicle.IsVehicleWindowIntact(veh, 2))
            {
            ...
            {
                VehicleBone("window_lr", "~y~Окно. ~r~Не исправно. ~r~[J]~s~~y~Починить~y~");
                Window[2] = true;
            }
            ...
            {
                VehicleBone("window_rr", "~y~Окно. ~r~Не исправно. ~r~[J]~s~~y~Починить~y~");
                Window[3] = true;
            }
            return Window;
        }

        public static void FixWindow(int veh)
        {
            int vehicle = veh;
            List<Vehicle> vehicles = Entities.Vehicles.All;
            Vehicle veh_obj = vehicles.Find(pl => pl.Handle == vehicle);
        ...
        public static float GetBodyHealth(int targetVeh)
        {
            int veh = targetVeh;
            ...
            {
                VehicleBone("bodyshell", "~y~Корпус~y~. ~y~Повереждён ~r~[Y] ~s~ ~y~Починить~y~" + bodyHealth.ToString());
            }
            else if (bodyHealth <= 400)
            {
                VehicleBone("bodyshell", "~y~Корпус~y~. ~r~Неисправен ~r~[Y] ~s~ ~y~Починить~y~" + bodyHealth.ToString());
            }
            return bodyHealth;
        }
        public static void FixBody(int veh, int health, int curHealth, int TotalBodyHealth, int totalMaxBodyHealth)
        {
            int vehicle = veh;
            List<Vehicle> vehicles = Entities.Vehicles.All;
            Vehicle veh_obj = vehicles.Find(pl => pl.Handle == vehicle);
            float bodyHealth = RAGE.Game.Vehicle.GetVehicleBodyHealth(vehicle);
            Vector3 pos = Player.LocalPlayer.Position;
            int index = 0;
            if (RAGE.Game.Entity.GetEntityModel(veh) == 841808271)
            {
                index = RAGE.Game.Entity.GetEntityBoneIndexByName(GetVehicle(5.0f), "boot");
            }
            else {index = RAGE.Game.Entity.GetEntityBoneIndexByName(vehicle, "bodyshell"); }
            ...
            if (RAGE.Game.Utils.Vdist(pos.X, pos.Y, pos.Z, coords.X, coords.Y, coords.Z) <= 2.0f)
            {
                ...
                {
                    RAGE.Game.Vehicle.SetVehicleBodyHealth(vehicle, health);
                ...
                if (health == 1000 && TotalBodyHealth != -1000 && curHealth <= 500)
                {
                    ...
                    RAGE.Game.Vehicle.SetVehicleBodyHealth(vehicle, health_veh);
                ...
                else if (TotalBodyHealth != -1000 && curHealth > 500)
                {
                    ...
                    RAGE.Game.Vehicle.SetVehicleBodyHealth(vehicle, totalMaxBodyHealth);
        ...
        public static void FixEngine(int veh, int health, int curHealth, int TotalHealth, int totalMaxHelth)
        {
            int vehicle = veh;
            List<Vehicle> vehicles = Entities.Vehicles.All;
            Vehicle veh_obj = vehicles.Find(pl => pl.Handle == vehicle);
            ...
            Vector3 pos = Player.LocalPlayer.Position;
            int index = RAGE.Game.Entity.GetEntityBoneIndexByName(vehicle, "engine");
            Vector3 coords = RAGE.Game.Entity.GetWorldPositionOfEntityBone(vehicle, index);
            if (RAGE.Game.Utils.Vdist(pos.X, pos.Y, pos.Z, coords.X, coords.Y, coords.Z) <= 2.0f)
            {
                if (health == 500 && curHealth < 500)
                {
                    uint vehicleModel = RAGE.Game.Entity.GetEntityModel(vehicle);
                    ...
                    RAGE.Game.Entity.SetEntityMaxSpeed(vehicle, maxSpeed);
                    RAGE.Game.Vehicle.SetVehicleEngineHealth(vehicle, health);
                ...
                if (health == 1000 && TotalHealth != -1000 && curHealth <= 500)
                {
                    ...
                    health_veh = health_veh - percent;
                    Chat.Output(health_veh.ToString());
                    RAGE.Game.Vehicle.SetVehicleEngineHealth(vehicle, health_veh);
                ...
                else if (TotalHealth != -1000 && curHealth > 500)
                {
                    ...
                    RAGE.Game.Vehicle.SetVehicleEngineHealth(vehicle, totalMaxHelth);
        ...
        public static void PlaceVehOnGround()
        {
            ...
            if (RAGE.Game.Entity.IsEntityAVehicle(veh))
            {
                RAGE.Game.Vehicle.SetVehicleOnGroundProperly(veh, 5);
            }
        }
        ...
        public static void VehicleBone(string bone, string text)
        {
            ...
            if (bone == "bodyshell" && RAGE.Game.Entity.GetEntityModel(veh) == 841808271)
            { 
                index = RAGE.Game.Entity.GetEntityBoneIndexByName(GetVehicle(5.0f), "boot");  
            }
        ...
        {
            Customs customs = Ticks_Mechs.CustomsCords.Find(c => c.NameStatic == name);
            // foreach (var item in Ticks_Mechs.CustomsCords)
            // {
            //      if(item.NameStatic == name)
            //    {
            Events.CallRemote("Add_New_Buisness", "", customs.NameStatic, customs.NameCustoms, nameBuis);
            // }
            //}
        ...
        public static void OpenTuningWindow()
        {
            if (!mechanic_client.Mechanic_Client.Open)
            {
                if (RAGE.Game.Ped.IsPedSittingInAnyVehicle(Player.LocalPlayer.Handle))
                {
                    int veh = RAGE.Game.Ped.GetVehiclePedIsUsing(Player.LocalPlayer.Handle);
                    uint modelVeh = RAGE.Game.Entity.GetEntityModel(veh);
                    Vector3 pos = Player.LocalPlayer.Position;
                    if (RAGE.Game.Entity.DoesEntityExist(veh) && (RAGE.Game.Vehicle.IsThisModelACar(modelVeh) || RAGE.Game.Vehicle.IsThisModelABike(modelVeh) || RAGE.Game.Vehicle.IsThisModelAQuadbike(modelVeh)))
                    {
                        ...
                        {
                            foreach (var item in mechanic_client.Ticks_Mechs.CustomsCords)
                            {
                                if (item.CustomCoords != null)
                                {
                                    if (RAGE.Game.Utils.Vdist(item.CustomCoords.X, item.CustomCoords.Y, item.CustomCoords.Z, pos.X, pos.Y, pos.Z) <= 5.0f && Math.Abs(pos.Z - item.CustomCoords.Z) <= 3.0f && Player.LocalPlayer.GetSharedData("typeCustoms").ToString() == item.NameCustoms)
                                    {
                                        ...
                                        mechanic_client.Mechanic_Browser.Create();
                                        mechanic_client.Mechanic_Client.StartCustomize();
                                        mechanic_client.Mechanic_Client.Open = true;
                                        mechanic_client.Mechanic_Client.Cursor = false;
                                        mechanic_client.Mechanic_Browser.Open("");
        ...
        private void ResetCamera(object[] args)
        {
            ...
            Notify("Нажмите F3 для редактирования");
        }
