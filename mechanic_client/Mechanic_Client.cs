...
namespace mechanic_client
{
    public class Mechanic_Client : Script
    {
        ...
        public Mechanic_Client()
        {
            ...
            Events.Add("DrawMarkers", DrawMarkers);
        ...
        private void SetDamagGoCar(object[] args)
        {
            ...
            string[] damag = args[1].ToString().Split(";");
                ...
                for (int i = 0; i < d1.Length; i++)
                {
                    if (d1[i])
                    {
                    Events.CallRemote("Sync_Wheel", i, v);
                ...
                for (int i = 0; i < d2.Length; i++)
                {
                    if (d2[i])
                    {
                    Events.CallRemote("Sync_Window", i, v);
       ...
       //}
        ...
        {
            int window_sync = Convert.ToInt32(args[0].ToString());
            List<Vehicle> vehicles = Entities.Vehicles.All;
            for (int i = 0; i < vehicles.Count; i++)
            {
                if (vehicles != null)
        ...
        {
            int body_sync = Convert.ToInt32(args[0].ToString());
            List<Vehicle> vehicles = Entities.Vehicles.All;
            for (int i = 0; i < vehicles.Count; i++)
            {
                if (vehicles != null)
        ...
        {
            int eng_sync = Convert.ToInt32(args[0].ToString());
            List<Vehicle> vehicles = Entities.Vehicles.All;
            for (int i = 0; i < vehicles.Count; i++)
            {
                if (vehicles != null)
        ...
        {
            int wheel_sync = Convert.ToInt32(args[0].ToString());
            List<Vehicle> vehicles = Entities.Vehicles.All;
            for (int i = 0; i < vehicles.Count; i++)
            {
                if (vehicles != null)
        ...
        static string[] windowParts = new string[4]
       {
        ...
        public static float GetEngineHealth()
        {
            ...
            {
                VehicleBone("engine", "~y~Двигатель~y~. ~y~Повереждён ~r~[Alt+Z] ~s~ ~y~Починить~y~");
            }
            else if (eHeatlh <= 400)
            {
                VehicleBone("engine", "~y~Двигатель~y~. ~r~Неисправен ~r~[Alt+Z] ~s~ ~y~Починить~y~");
            }
            return eHeatlh;
        }
        ...
        public static float GetBodyHealth()
        {
            ...
            {
                VehicleBone("bodyshell", "~y~Корпус~y~. ~y~Повереждён ~r~[Alt+Y] ~s~ ~y~Починить~y~");
            }
            else if (bodyHealth <= 400)
            {
                VehicleBone("bodyshell", "~y~Корпус~y~. ~r~Неисправен ~r~[Alt+Y] ~s~ ~y~Починить~y~");
            }
            return bodyHealth;
        }
        ...
        public static void FixEngine(int health, int curHealth, int TotalHealth, int totalMaxHelth)
        {
            ...
            if (RAGE.Game.Utils.Vdist(pos.X, pos.Y, pos.Z, coords.X, coords.Y, coords.Z) <= 2.0f)
            {
                ...
                if (health == 1000 && TotalHealth != -1000 && curHealth <= 500)
                {
                     ...
                     //Chat.Output(RAGE.Game.Vehicle.GetVehicleNumberPlateText(veh) + " " + RAGE.Game.Vehicle.GetDisplayNameFromVehicleModel(RAGE.Game.Entity.GetEntityModel(veh)).ToLower()+" "+ TotalHealth);
                ...
                else if (TotalHealth != -1000 && curHealth > 500)
                {
                    ...
                    Events.CallRemote("Sync_Eng", veh_obj, TotalHealth);
                    Events.CallRemote("Save_Car_Health", veh_obj, TotalHealth);
